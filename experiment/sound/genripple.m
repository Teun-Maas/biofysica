function [snd,Fs] = genripple(vel,dens,md,durrip,durstat,varargin)
% [SND,FS] = GENRIPPLE(VEL,DENS,MOD,DURDYN,DURSTAT)
%
...
if durstat>0 % if required, construct a static part of the noise, and prepend
	...
	% Normalize ripple power to static power - old stuff
...
subplot(223)
% col = statcolor(64,[],[],[],'def',1,'disp',false);
% col = flipud(col);
% colormap(col)
col = jet(64);
nsamples	= length(snd);
