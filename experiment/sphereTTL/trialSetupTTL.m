function [stim,cfg,stimttl] = trialSetup(cfg,stim)
% HLED = RUNTRIAL(RA16)
%
...
if any(selled)
	...
	%%
	cnt		= 0;
    ontime  = 999;      % will be ignored stimttlfun, ttl = 0 by default.
	for ledIdx = 1:nled
		% TDT RA16
...
%         cfg.RA16_1.SetTagVal(str3,1); % HEADTRIGGER
        ...
        if ledIdx > 1
            ontime = led(ledIdx).offdelay+1 - led(ledIdx).ondelay+1;
        end
    ...
    stim(find(selled,1)).ledhandle.set_stimttl(0);
    
    stimttl.ledhandle = stim(find(selled,1)).ledhandle;
    stimttl.flash     = stimttlfun(ontime);
	%%
end
