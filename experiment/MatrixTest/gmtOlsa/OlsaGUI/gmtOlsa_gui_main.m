function varargout = gmtOlsa_gui_main(varargin)
% GMTOLSA_GUI_MAIN MATLAB code for gmtOlsa_gui_main.fig
%      GMTOLSA_GUI_MAIN, by itself, creates a new GMTOLSA_GUI_MAIN or raises the existing
%      singleton*.
%
%      H = GMTOLSA_GUI_MAIN returns the handle to a new GMTOLSA_GUI_MAIN or the handle to
%      the existing singleton*.
%
%      GMTOLSA_GUI_MAIN('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GMTOLSA_GUI_MAIN.M with the given input arguments.
%
%      GMTOLSA_GUI_MAIN('Property','Value',...) creates a new GMTOLSA_GUI_MAIN or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before gmtOlsa_gui_main_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to gmtOlsa_gui_main_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help gmtOlsa_gui_main

% Last Modified by GUIDE v2.5 09-Aug-2021 15:06:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @gmtOlsa_gui_main_OpeningFcn, ...
                   'gui_OutputFcn',  @gmtOlsa_gui_main_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before gmtOlsa_gui_main is made visible.
function gmtOlsa_gui_main_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gmtOlsa_gui_main (see VARARGIN)
% Choose default command line output for gmtOlsa_gui_main
handles.output = hObject;

filename = 'default.mat';
path = gmtOlsa_getPathSettings;
gmtOlsa_loadSettings(handles, fullfile(path.SETTINGS, filename));

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes gmtOlsa_gui_main wait for user response (see UIRESUME)
% uiwait(handles.fig_main);


% --- Outputs from this function are returned to the command line.
function varargout = gmtOlsa_gui_main_OutputFcn(hObject, eventdata, handles) 
...
if ~isempty(stratFile)
    gmtOlsa_loadSavedStrategy(stratFile, 1);
else
...
if filename
    pathname = strrep(pathname, pwd, ['.']);
    try
        gmtOlsa_loadSettings(handles, [pathname, filename]);
    catch ex
...
if filename
    pathname = strrep(pathname, pwd, '.');
    try
        gmtOlsa_saveSettings(handles, [pathname, filename]);
    catch ex
