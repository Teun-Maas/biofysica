function s = gmtOlsa_getSettings(guiHandles)
...
%#########################################################################
    ...
    s.path         = gmtOlsa_getPathSettings;
...
%#########################################################################        
    ...
    function constants = getConstants
        ...
        constants.parserRowOffset       = 4;
        constants.SRT_nTrials           = 10;
    ...
    function user = getUserSettings(h, s)                
        ...
        user.noiseFile          = strcat(s.path.SOUNDS,'\', nFname, nFext);
        ...
        % HEADPHONES SETUP - SIDE - NH / VOC
        user.sideOn_L           = get(h.checkbox18,'Value');
        user.sideOn_R           = get(h.checkbox19,'Value');
        user.vocoderOn_L        = get(h.checkbox16,'Value');
        user.vocoderOn_R        = get(h.checkbox17,'Value');

        user.pupil              = get(h.pupil_dilation,'Value'); %RL
        ...
        assert(max([user.sideOn_L, user.sideOn_R]) > 0, 'At least one side should be selected');
        assert(min([user.sideOn_L, user.sideOn_R] - [user.vocoderOn_L, user.vocoderOn_R]) >= 0, 'The combination of sides and vocoders is not valid'); 
