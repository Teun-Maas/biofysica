function matrix_PlayOverHeadphones(rz6, settings, signals)  

%#########################################################################
%#                                                                       #
%#                                                                       #
%#                      matrix_PlayOverHeadphones                        #
%#                                                                       #
%#                                                                       #
%#########################################################################    
    ...
    matrix_prepareRZ6(rz6, getRZ6Settings(signals, settings));  
...
%#########################################################################                                  
    ...
    function startPupillometry(s)
        ...
        function fn = getFilenamePupil(s)
            speech_set    = [s.user.speechOn_L, s.user.speechOn_R];
            vocoder_set = [s.user.vocoderOn_L s.user.vocoderOn_R];
            SNR = s.user.speechLevel_dB-s.user.noiseLevel_dB;
            fname_pupil = [s.user.subjectId '_speech' num2str(speech_set) '_voc' num2str(vocoder_set) '_snr' num2str(SNR)];
    ...
    function showScreen(settings, i)        
        matrix_ShowScreenAttr(settings.screen, i);        
    end %showScreen
