function varargout = slims_calibrate(varargin)
% Check fixation and train calibration networks
%
% SLIMS_CALIBRATE(FNAME)
%
%  SLIMS_CALIBRATE
%
...
% Last Modified by GUIDE v2.5 21-Jul-2022 15:17:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @slims_calibrate_OpeningFcn, ...
	'gui_OutputFcn',  @slims_calibrate_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before slims_calibrate is made visible.
function slims_calibrate_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure

% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to slims_calibrate (see VARARGIN)

% Choose default command line output for slims_calibrate
...
% UIWAIT makes slims_calibrate wait for user response (see UIRESUME)
...
if length(varargin)==1
	...
	handles.fname                   = pa_fcheckext(handles.fname);
	handles.fname                   = pa_fcheckexist(handles.fname);
end
...
% Select Fixation LED (LED = type 0, Sky = 1)
%%
sel                                 = ismember(handles.Stim(:,3),0);
S                                   = handles.Stim(sel,:);
mn                                  = min(unique(S(:,2))); % first stimulus
sel                                 = ismember(S(:,2),mn);
...
function varargout = slims_calibrate_OutputFcn(hObject, eventdata, handles)
...
% 2015: changed to work with hoop and sphere
...
if ~isempty(d)
	setup = 'sphere';
	disp('PA_LOG2STIM assumes you have recorded from the SPHERE lab');
else
	% Hoop
