function [AZ,EL] = hoopXY2azel(X,Y)
% FART2AZEL Transfrom HOOP to double polar coordinates.
% [AZ,EL] = FART2AZEL(X,Y) transforms corresponding elements of data
%     stored in HOOP (Fast Auditory Rotating Target) coordinates X 
...
%       HOOP coordinates *does not* directly reflect Azimuth-Elevation
...
%       the double polar system irrespective of HOOP rotation angle
%       Theta).
%       Azimuth AZ can be deduced by applying the formula:
%
%           AZ = arcsin( sin(Theta) * cos(EL) )
%
%       where Theta = HOOP rotation angle (deg), and EL = elevation angle
%       (deg).
%
...
% i.e. the front speakers (1-31) will be positioned in the rear when HOOP has
