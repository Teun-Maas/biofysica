function varargout = spherePrime_red(varargin)
% SPHEREPRIME_RED MATLAB code for spherePrime_red.fig
% Default colour of LEDS is red
%      SPHEREPRIME_RED, by itself, creates a new SPHEREPRIME_RED or raises the existing
%      singleton*.
%
%      H = SPHEREPRIME_RED returns the handle to a new SPHEREPRIME_RED or the handle to
%      the existing singleton*.
%
%      SPHEREPRIME_RED('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPHEREPRIME_RED.M with the given input arguments.
%
%      SPHEREPRIME_RED('Property','Value',...) creates a new SPHEREPRIME_RED or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before spherePrime_red_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to spherePrime_red_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help spherePrime_red

% Last Modified by GUIDE v2.5 20-Sep-2016 13:43:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @spherePrime_red_OpeningFcn, ...
	'gui_OutputFcn',  @spherePrime_red_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before spherePrime_red is made visible.
function spherePrime_red_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to spherePrime_red (see VARARGIN)
...
% Choose default command line output for spherePrime_red
handles.output	= hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes spherePrime_red wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = spherePrime_red_OutputFcn(hObject, eventdata, handles)
...
if any(selled)
	...
	for ledIdx = 1:nled
		...
		for ii	= 1:2:n
			cnt = cnt+1;
			s(cnt).set(led(ledIdx).Z,'r');
			s(cnt).intensity('r',round(led(ledIdx).intensity/6)); % hoop: range 0-255, sphere range 1-50
		end
	end
...
%% Read DATA
% warning('spherePrime_red now checks each trial whether data should be acquired'); % this warning should be removed
