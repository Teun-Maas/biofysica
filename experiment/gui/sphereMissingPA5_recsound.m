function varargout = sphereMissingPA5_recsound(varargin)
% SPHEREMISSINGPA5_RECSOUND MATLAB code for sphereMissingPA5_recsound.fig
%      SPHEREMISSINGPA5_RECSOUND, by itself, creates a new SPHEREMISSINGPA5_RECSOUND or raises the existing
%      singleton*.
%
%      H = SPHEREMISSINGPA5_RECSOUND returns the handle to a new SPHEREMISSINGPA5_RECSOUND or the handle to
%      the existing singleton*.
%
%      SPHEREMISSINGPA5_RECSOUND('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPHEREMISSINGPA5_RECSOUND.M with the given input arguments.
%
%      SPHEREMISSINGPA5_RECSOUND('Property','Value',...) creates a new SPHEREMISSINGPA5_RECSOUND or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sphereMissingPA5_recsound_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sphereMissingPA5_recsound_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sphereMissingPA5_recsound

% Last Modified by GUIDE v2.5 04-Jan-2016 16:09:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @sphereMissingPA5_recsound_OpeningFcn, ...
	'gui_OutputFcn',  @sphereMissingPA5_recsound_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before sphereMissingPA5_recsound is made visible.
function sphereMissingPA5_recsound_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sphereMissingPA5_recsound (see VARARGIN)
...
% Choose default command line output for sphereMissingPA5_recsound
handles.output	= hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes sphereMissingPA5_recsound wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = sphereMissingPA5_recsound_OutputFcn(hObject, eventdata, handles)
...
expIdx				= get(handles.popupmenu_expinitials,'Value');
cfg.sndrec          = get(handles.popupmenu5,'Value');
...
cfg.recdataidx		= {'recData1' 'recData2'}; % names of Data sources
...
% Read RP2 DATA
if cfg.sndrec==1
RP2_data	= NaN(cfg.RecSoundSamples+1000,2);
t = tic;
for ii		= 1:cfg.nRP2
	RP2_data(:,ii) = cfg.RP2_1.ReadTagV(cfg.recdataidx{ii},0,cfg.RecSoundSamples+1000)';
end

data(1).sndrec  = RP2_data;
else
    data(1).sndrec = 'No sound recorded';
end
