function varargout = sphereMissingPA5_buttonbox(varargin)
% SPHEREMISSINGPA5_BUTTONBOX MATLAB code for sphereMissingPA5_buttonbox.fig
%      SPHEREMISSINGPA5_BUTTONBOX, by itself, creates a new SPHEREMISSINGPA5_BUTTONBOX or raises the existing
%      singleton*.
%
%      H = SPHEREMISSINGPA5_BUTTONBOX returns the handle to a new SPHEREMISSINGPA5_BUTTONBOX or the handle to
%      the existing singleton*.
%
%      SPHEREMISSINGPA5_BUTTONBOX('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPHEREMISSINGPA5_BUTTONBOX.M with the given input arguments.
%
%      SPHEREMISSINGPA5_BUTTONBOX('Property','Value',...) creates a new SPHEREMISSINGPA5_BUTTONBOX or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sphereMissingPA5_buttonbox_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sphereMissingPA5_buttonbox_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sphereMissingPA5_buttonbox

% Last Modified by GUIDE v2.5 24-Nov-2015 12:04:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @sphereMissingPA5_buttonbox_OpeningFcn, ...
	'gui_OutputFcn',  @sphereMissingPA5_buttonbox_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before sphereMissingPA5_buttonbox is made visible.
function sphereMissingPA5_buttonbox_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sphereMissingPA5_buttonbox (see VARARGIN)
...
% Choose default command line output for sphereMissingPA5_buttonbox
handles.output	= hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes sphereMissingPA5_buttonbox wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = sphereMissingPA5_buttonbox_OutputFcn(hObject, eventdata, handles)
...
%% Note
cfg.note	= {'This is a script for a missing PA54 with buttonbox functionality',date};
...
if ~isfield(cfg,'RP2_3circuit')
	% RP2_2circuit = 'C:\matlab\Marc\Experiment\RPvdsEx\hrtf_2Channels_rp2.rco';
	% RP2_2circuit = 'C:\Gitlab\Marc\experiment\RPvdsEx\GvB_GWN_Single_Sound.rcx';
	cfg.RP2_3circuit = which('sphere_rp2_buttonbox.rcx');
end
...
[~,cfg.sRP2_3circuit]	= fileparts(cfg.RP2_3circuit);
...
[RP2_3, err(8)]		= RP2(3,handles.cfg.RP2_3circuit); % Real-time processor 3
...
handles.cfg.RP2_3Status		= RP2_3.GetStatus;
...
%% Led light on Buttonbox on 

% cfg.RP2_3.SoftTrg(1); % switch on light
...
if strcmp(cfg.sRP2_1circuit,'sphere_RP2_WAV')
	disp('Sounds are obtained from MAT file but sound like donkeys');
	fname		= fullfile(cfg.snddir,snd.matfile);
...
cfg.RP2_3.SoftTrg(1) % trigger via RP2_3
% pause(.1); % short break
...
%% Data acquisition
disp('Waiting for data acquisition and button response');
% pause(.1); % short break
while cfg.RP2_3.GetTagVal('ExpActive')==1
    % do nothing while buttonbox button 1/4 are not pressed to indicate
    % perception.
end
...
%% read RP2_3 reaction times from buttonbox 
Fs = 48828.125000;
RT = cfg.RP2_3.ReadTagV('RTOut',cfg.trial-1,1);
RT = RT/Fs*1000; % reaction time in ms
Button1 = cfg.RP2_3.GetTagVal('Bit1MATLAB');
Button4 = cfg.RP2_3.GetTagVal('Bit4MATLAB');
data(1).RT = RT;
data(1).Button1 = Button1;
data(1).Button4 = Button4;
...
%% Reset lights/counter on buttonbox/RP2_3
cfg.RP2_3.SoftTrg(2);
