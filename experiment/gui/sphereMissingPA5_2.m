function varargout = sphereMissingPA5_2(varargin)
% SPHEREMISSINGPA5_2 MATLAB code for sphereMissingPA5_2.fig
%      SPHEREMISSINGPA5_2, by itself, creates a new SPHEREMISSINGPA5_2 or raises the existing
%      singleton*.
%
%      H = SPHEREMISSINGPA5_2 returns the handle to a new SPHEREMISSINGPA5_2 or the handle to
%      the existing singleton*.
%
%      SPHEREMISSINGPA5_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPHEREMISSINGPA5_2.M with the given input arguments.
%
%      SPHEREMISSINGPA5_2('Property','Value',...) creates a new SPHEREMISSINGPA5_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sphereMissingPA5_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sphereMissingPA5_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sphereMissingPA5_2

% Last Modified by GUIDE v2.5 02-Sep-2016 14:28:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @sphereMissingPA5_2_OpeningFcn, ...
	'gui_OutputFcn',  @sphereMissingPA5_2_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before sphereMissingPA5_2 is made visible.
function sphereMissingPA5_2_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sphereMissingPA5_2 (see VARARGIN)
...
% Choose default command line output for sphereMissingPA5_2
handles.output	= hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes sphereMissingPA5_2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = sphereMissingPA5_2_OutputFcn(hObject, eventdata, handles)
...
if strcmp(cfg.sRP2_1circuit,'sphere_RP2_WAV')
   ...
   if strcmp(RP2str,'RP2_1')
	...
	cfg.(RP2str).SetTagVal(['bufferSize1'],maxSamples);
	cfg.(RP2str).WriteTagV(['wavData1'], 0,sig(1:maxSamples));
	cfg.(RP2str).SetTagVal(['chanEnable1'],1);
   else
...
axes(handles.axes_xy); %#ok<*NASGU>
% if isfield(handles.cfg,'hcurtar')     % commented out by Rachel
...
%% plot
% if isfield(handles.cfg,'hcurdat')         % commented out by Rachel
...
%% Read DATA
% warning('sphereMissingPA5_2 now checks each trial whether data should be acquired'); % this warning should be removed
