function varargout = spheretestled(varargin)
% SPHERETESTLED MATLAB code for spheretestled.fig
%      SPHERETESTLED, by itself, creates a new SPHERETESTLED or raises the existing
%      singleton*.
%
%      H = SPHERETESTLED returns the handle to a new SPHERETESTLED or the handle to
%      the existing singleton*.
%
%      SPHERETESTLED('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPHERETESTLED.M with the given input arguments.
%
%      SPHERETESTLED('Property','Value',...) creates a new SPHERETESTLED or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before spheretestled_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to spheretestled_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help spheretestled

% Last Modified by GUIDE v2.5 04-Nov-2015 12:59:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @spheretestled_OpeningFcn, ...
	'gui_OutputFcn',  @spheretestled_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before spheretestled is made visible.
function spheretestled_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to spheretestled (see VARARGIN)
...
% Choose default command line output for spheretestled
handles.output	= hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes spheretestled wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = spheretestled_OutputFcn(hObject, eventdata, handles)
...
disp('----')
		...
		name = [(2*ledIdx-1) (2*ledIdx)]
		value = [led(ledIdx).onevent+1 led(ledIdx).offevent+1]
		...
		delay = [led(ledIdx).ondelay+1 led(ledIdx).offdelay+1]
