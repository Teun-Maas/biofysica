function varargout = spherePrime_buttonbox(varargin)
% SPHEREPRIME_BUTTONBOX MATLAB code for spherePrime_buttonbox.fig
%      SPHEREPRIME_BUTTONBOX, by itself, creates a new SPHEREPRIME_BUTTONBOX or raises the existing
%      singleton*.
%
%      H = SPHEREPRIME_BUTTONBOX returns the handle to a new SPHEREPRIME_BUTTONBOX or the handle to
%      the existing singleton*.
%
%      SPHEREPRIME_BUTTONBOX('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPHEREPRIME_BUTTONBOX.M with the given input arguments.
%
%      SPHEREPRIME_BUTTONBOX('Property','Value',...) creates a new SPHEREPRIME_BUTTONBOX or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before spherePrime_buttonbox_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to spherePrime_buttonbox_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help spherePrime_buttonbox

% Last Modified by GUIDE v2.5 04-Dec-2015 15:55:11

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @spherePrime_buttonbox_OpeningFcn, ...
	'gui_OutputFcn',  @spherePrime_buttonbox_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before spherePrime_buttonbox is made visible.
function spherePrime_buttonbox_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to spherePrime_buttonbox (see VARARGIN)
...
% Choose default command line output for spherePrime_buttonbox
handles.output	= hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes spherePrime_buttonbox wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = spherePrime_buttonbox_OutputFcn(hObject, eventdata, handles)
...
[RP2_3, err(9)]		= RP2(3,handles.cfg.RP2_3circuit); % Real-time processor 3
...
%% read recorded sound from RP2_1 channel 1 (for hrtf measurement)
% data(1).RP2sndrec = cfg.RP2_1.ReadTagV(1,0,cfg.MaxSamples)';
%% Save trial data
