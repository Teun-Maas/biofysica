...
%xml2struct can be found at Matlab File Exchange

%CY Y (2021). xml2struct , with bug fix and added features (https://github.com/joe-of-all-trades/xml2struct), GitHub. Retrieved August 25, 2021.

    filename=tempname;
    try
        fileID = fopen(filename,'w');
        fwrite(fileID,streaminfo.as_xml());
        fclose(fileID);
    catch
        error(message('biofysica:lsl_xml_ptr2struct:FileReadError', filename));
    end
    
    try
        outStruct=lsl_xml2struct(filename);
    catch ME
        delete(filename);
        throw(ME);
    end
...
function  outStruct  = lsl_xml2struct(input)
%lsl_xml2struct converts xml file into a MATLAB structure
%
% outStruct = lsl_xml2struct2(input)
%
% lsl_xml2struct2 takes either a java xml object, an xml file, or a string in
