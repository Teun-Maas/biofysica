function varargout = spherePrime(varargin)
% SPHEREPRIME MATLAB code for spherePrime.fig
%      SPHEREPRIME, by itself, creates a new SPHEREPRIME or raises the existing
%      singleton*.
%
%      H = SPHEREPRIME returns the handle to a new SPHEREPRIME or the handle to
%      the existing singleton*.
%
%      SPHEREPRIME('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPHEREPRIME.M with the given input arguments.
%
%      SPHEREPRIME('Property','Value',...) creates a new SPHEREPRIME or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before spherePrime_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to spherePrime_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help spherePrime

% Last Modified by GUIDE v2.5 14-Jul-2015 08:56:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @spherePrime_OpeningFcn, ...
	'gui_OutputFcn',  @spherePrime_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
% --- Executes just before spherePrime is made visible.
function spherePrime_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to spherePrime (see VARARGIN)
...
% Choose default command line output for spherePrime
handles.output	= hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes spherePrime wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = spherePrime_OutputFcn(hObject, eventdata, handles)
