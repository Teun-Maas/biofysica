...
modelname = which('jags_singleomega_model.txt');
% modelname = '/Users/marcw/Documents/GitLab/biofysica/bayesdataanalysis/psychophysics/jags_single_omega_model.txt';
%% MCMC parameters
chain_globals;
...
parameters			= {'alpha','sigma','thresh','theta','omega'};
thresh				= NaN(nLevels-1,1);
thresh(1)			= 1 + 0.5;
thresh(nLevels-1)	= nLevels-1 + 0.5;
dataStruct			= struct('x',x,'y',y,'Ntotal',Ntotal,'nYlevels',nLevels,'thresh',thresh); % data structure
initsStruct			= initialize(nChains,x,y); % initial parameter values
...
function initsStruct	= initialize(nChains,x,y)
initsStruct = struct([]);
...
for ii = 1:nChains
	...
	initsStruct(ii).omega			= std(x);  % standardized std
	initsStruct(ii).theta			= x(idx);  % standardized std
	initsStruct(ii).alpha			= mx;  % standardized std
	initsStruct(ii).sigma			= mean(S);  % standardized std
