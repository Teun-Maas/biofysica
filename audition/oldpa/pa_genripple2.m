function [snd,Fs] = pa_genripple2(vel,dens,md,durrip,durstat,varargin)
...
for m = 1:fEnvSize
	f_ind				= round(fr(m)*durrip);
	S_tmpA				= fftshift(fft(A(m,:)))*exp(1i*th(m))/tEnvSize*nTime/2;
	
	padzerosonleft		= f_ind - tEnvSize2 - 1;
	padzerosonright		= nTime/2 - f_ind - tEnvSize2;
