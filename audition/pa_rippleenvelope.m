function pa_rippleenvelope(vel,dens,md,durrip,varargin)
...
switch meth
	case 'time'
		[rip,env,T,F]		= genrip(vel,dens,md,durrip,Fs,nFreq,Freq,Oct,Phi);
	case 'fft'
...
whos env T F time Freq
subplot(222)

f		= pa_oct2bw(250,Oct);

imagesc(time,Oct,env)
set(gca,'YDir','normal');
colorbar;
...
set(gca,'YTick',1:6);
set(gca,'YTickLabel',pa_oct2bw(250,1:6)/1000);
%% Play
if strcmpi(plee,'y');
...
function [snd,A,T,F] = genrip(vel,dens,md,durrip,Fs,nFreq,Freq,Oct,Phi)
nTime   = round( (durrip/1000)*Fs ); % # Samples for Rippled Noise
time	= ((1:nTime)-1)/Fs; % Time (sec)
...
A = [];
for ii = 1:nFreq
	Phistat = Phi(ii); % phase after stationary period needs to be included
	a		= 1+md*sin(T+F(ii));
