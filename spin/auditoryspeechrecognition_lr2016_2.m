function auditoryspeechrecognition_lr2016_2(varargin)
% MCMC = BINORATE(Z,N,S)
%
...
sampleFlag		= keyval('sample',varargin,false);
...
% 	keyboard
	...
	%% Loop
	for ii = 1:np % for every subject
		for jj = 1:nw % for every word
			for kk = 1:nl % for every list
				for ll = 1:nsnr % for every signal-to-noise ratio
					selsub		= p==up(ii);
					selword		= w==uw(jj);
					sellist		= l==ul(kk);
					selsnr		= snr==usnr(ll);
					sel			= selsub & selword & selsnr' & sellist;
					
					co			= unique(o(selword)); % current order
					selorder	= o==co;
					selresp		= strcmp(uwords(uw(jj)),r)';
					selstim		= strcmp(uwords(uw(jj)),t)';
					selfalse	= selsub & sellist & selresp & ~selstim & selorder' &selsnr';
					
					sum(y(sel))
					z(ii,jj,kk,ll)		= sum(y(sel));
					N(ii,jj,kk,ll)		= sum(sel);
					g(ii,jj,kk,ll)		= sum(selfalse);
					Ng(ii,jj,kk,ll)		= sum(selsub & sellist & ~selstim & selorder' & selsnr');
					xSNR(ii,jj,kk,ll)	= usnr(ll);
					xP(ii,jj,kk,ll)		= ii;
					xW(ii,jj,kk,ll)		= jj;
					xL(ii,jj,kk,ll)		= kk;
...
if sampleFlag
	...
	%% Actual regression
	samples				= genMCMC(z,N,g,Ng,xP,xW,xL,xSNR,...
...
function [samples,stats] = genMCMC(z,N,g,Ng,xP,xW,xL,xSNR,varargin)
% B = GENMCMC(X,Y)
%
...
parameters		= {'ptheta','wtheta','ltheta',...
	'pomega','womega','lomega',...
	'gamma'};
writemodel;
...
dataStruct = struct('z',z,'N',N,'np',np,'nw',nw,'nl',nl,'g',g,'Ng',Ng,...
	'x',xSNR,'p',xP,'w',xW,'l',xL,...
	'nTotal',numel(z));
...
str = [
	...
	'\t g[i] ~ dbin(gamma[p[i],w[i]],Ng[i]) \r\n',...
	'\t# Probability Correct Is Product Of Word By Person Rates \r\n',...
	'\t theta[i]  <- (1-gamma[p[i],w[i]])*f[i] +gamma[p[i],w[i]]\r\n',...
	'\t	f[i]	<- ilogit( (2*log(1/alpha-1))/(pomega[p[i]]+womega[w[i]]+lomega[l[i]]) * ( x[i]-ptheta[p[i]]-wtheta[w[i]]-ltheta[l[i]] ) )  \r\n',...
	'\t} \r\n',...
