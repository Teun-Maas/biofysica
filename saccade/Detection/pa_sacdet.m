...
%  handles = plotsac(handles);
%  handles = plottrace(handles);
 

% function handles = plotsac(handles)
...
function handles = plottrace(handles)
linkaxes([handles.axes_trace handles.axes_velocity],'x');
%% Get Variables from Handles Structure
ctrial                                 = handles.ctrial;
det                                    = handles.det;
...
if ~(ctrial == max(handles.Ntrial) && csaccade == ntmp)     % unless current saccade is last saccade of experimental run
    ...
    elseif  isempty(nsaccade) || csaccade == max(nsaccade);      % however, if current saccade is the last saccade of the trial
        ...
        ctrial                      = handles.ctrial;
        sel                        = trialnr==ctrial;    % Find which saccade-indices belong to current trial
        handles.onoff(5,sel)       = 1;
    end
elseif (ctrial == max(handles.Ntrial) && csaccade == ntmp)
    load handel;
    sound(y,Fs);
    menu_save_Callback(hObject, eventdata, handles);
    return
...
function btn_prevsac_Callback(hObject, eventdata, handles) %#ok<*DEFNU>
...
function edit14_Callback(hObject, eventdata, handles) %#ok<*INUSD>
